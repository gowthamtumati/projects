...
def depthFirstSearch(problem):
    ...
    while not stateStack.isEmpty():
        ...
        for s in successorState:
            if not  s[0] in visitedList:
                currentState = s[0]
                stateStack.push((s[0], actions + [s[1]]))
    ...
    while not pq.isEmpty():
        ...
        if head[0] not in visitedList:
            ...
            for successor_nodes, act, co in problem.getSuccessors(head[0]):
                if successor_nodes and successor_nodes not in visitedList:
                    pq.push((successor_nodes, head[1] + [act], head[2] + co), head[2] + co)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not pq.isEmpty():
        ...
        if head[0] not in visitedList:
            ...
            for successor_nodes, act, co in problem.getSuccessors(head[0]):
                    if successor_nodes and successor_nodes not in visitedList:
                        pq.push((successor_nodes, head[1] + [act], head[2] + co), head[2] + co + heuristic(successor_nodes, problem))
